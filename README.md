...
* **灵活配置**：图形化配置窗口，支持 Drag & Drop 调整按键位置，独立参数面板窗口实时编辑属性。
* **多选编辑**：Ctrl+点击多选按键，批量移动/调整大小/颜色/透明度/瀑布流宽度。
* **物理吸附**：拖拽按键时自动对齐，碰撞避免重叠。
* **Profile 管理**：多配置一键切换，支持新增/重命名/删除 profile。
...
- **Ctrl+点击**：多选按键
- **独立参数面板**：浮动窗口编辑 X/Y 坐标、宽度/高度、激发颜色、透明度、瀑布流宽度百分比
- **多选编辑**：选中的按键可批量修改上述属性
- **Delete 键 / 按钮**：删除选中按键（带确认弹窗，Enter 确认，ESC 取消）
- **+ Add Key**：添加新按键（按下键盘任意键绑定）
- **Profile 管理**：左侧列表切换配置方案、新增/重命名/删除 profile

### 配置说明

配置保存在 `configs/profiles/` 目录下，以 JSON 格式存储，支持多 profile 管理。
通过 `configs/.active` 文件记录当前激活的 profile。
可通过配置窗口图形化调整。各项参数包括按键布局、颜色、瀑布流方向与速度等。
